            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑选课</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="kechengId" name="kechengId" type="hidden">
                                    <div class="form-group col-md-6 xuankeUuidNumberDiv">
                                        <label>选课编号</label>
                                        <input style="width: 450px" id="xuankeUuidNumber" name="xuankeUuidNumber" class="form-control"
                                               placeholder="选课编号">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("xuanke/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addxuanke');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                xueshengId: "required",
                kechengId: "required",
                xuankeUuidNumber: "required",
            },
            messages: {
                xueshengId: "学生不能为空",
                kechengId: "课程不能为空",
                xuankeUuidNumber: "选课编号不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addxuanke = window.sessionStorage.getItem("addxuanke");
        if (addxuanke != null && addxuanke != "" && addxuanke != "null") {
            //注册表单验证
            $(validform());
            $("#xuankeUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("xuanke/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#xuankeUuidNumber").val(ruleForm.xuankeUuidNumber);
