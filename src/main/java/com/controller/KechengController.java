    ...
    */
    ...
    public R save(@RequestBody KechengEntity kecheng, HttpServletRequest request){
        ...
        Wrapper<KechengEntity> queryWrapper = new EntityWrapper<KechengEntity>()
            ...
            .eq("kecheng_daima", kecheng.getKechengDaima())
            .eq("yuanxi_types", kecheng.getYuanxiTypes())
            .eq("kecheng_xingzhi_types", kecheng.getKechengXingzhiTypes())
            .eq("kecheng_xuefen", kecheng.getKechengXuefen())
            .eq("kecheng_zongxueshi", kecheng.getKechengZongxueshi())
            .eq("kecheng_kaohe_types", kecheng.getKechengKaoheTypes())
            .eq("kecheng_renshu", kecheng.getKechengRenshu())
            ;
...
//            kecheng.setLaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R xuanke(@RequestParam("id") Integer id, HttpServletRequest request){
        logger.debug("xuanke:,,Controller:{},,id:{}",this.getClass().getName(),id.toString());
        KechengEntity kechengEntity = kechengService.selectById(id);
        if(kechengEntity == null)
            return R.error("查不到课程");
        XuankeEntity xuankeEntity = xuankeService.selectOne(
                new EntityWrapper<XuankeEntity>()
                        .eq("xuesheng_id", Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))))
                        .eq("kecheng_id", id)
        );
        if(xuankeEntity != null)
            return R.error("当前学生已经选课当前课程,无法重复选当前课程");


        List<XuankeEntity> xuankeEntities = xuankeService.selectList(
                new EntityWrapper<XuankeEntity>()
                        .eq("kecheng_id", id)
        );

        if(xuankeEntities.size()>=kechengEntity.getKechengRenshu())
            return R.error("当前课程选课人数已满,请选择其他课程");

        XuankeEntity xuankeEntity1 = new XuankeEntity();
        xuankeEntity1.setCreateTime(new Date());
        xuankeEntity1.setInsertTime(new Date());
        xuankeEntity1.setXueshengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        xuankeEntity1.setKechengId(id);
        xuankeEntity1.setXuankeUuidNumber(String.valueOf(new Date().getTime()));
        xuankeService.insert(xuankeEntity1);
        return R.ok();
    }
...
//                            kechengEntity.setKechengDaima(data.get(0));                    //课程代码 要改的
//                            kechengEntity.setYuanxiTypes(Integer.valueOf(data.get(0)));   //院系 要改的
//                            kechengEntity.setKechengXingzhiTypes(Integer.valueOf(data.get(0)));   //课程性质 要改的
//                            kechengEntity.setKechengXuefen(Integer.valueOf(data.get(0)));   //课程学分 要改的
//                            kechengEntity.setKechengZongxueshi(Integer.valueOf(data.get(0)));   //总学时 要改的
//                            kechengEntity.setKechengKaoheTypes(Integer.valueOf(data.get(0)));   //考核方式 要改的
//                            kechengEntity.setKechengRenshu(Integer.valueOf(data.get(0)));   //最大选课人数 要改的
//                            kechengEntity.setJiezhiTime(sdf.parse(data.get(0)));          //选课截止时间 要改的
